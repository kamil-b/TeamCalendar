...
public class Application {
	...
	//Board board1 = new Board("neutrino");
...
/*
	@Bean
	public CommandLineRunner demoEvent(UserRepository repository) {
		return (args) -> {
			User kamil = new User("kamil", "123");
			kamil.setSuperior("monika");
			kamil.setRole(JobRole.DEVELOPER);
			kamil.setEmail("kamil.bednarczyk@ericsson.com");
			board1.addUserToBoard(kamil);
			repository.save(kamil);
			User kamil2 = new User("kamil2", "123");
			kamil2.addBoardToList(board1);
			repository.save(kamil2);
			
			User monika = new User("monika" , "123");
			monika.setRole(JobRole.LINE_MANAGER);
			repository.save(monika);
		};

	}*/
/*
	@Bean
	public CommandLineRunner demo(TipRepository repository) {
		return (args) -> {
			...
			repository.save(new Tip("Git", "sfsaf asfs afas f", "asdfas a  asfas ", "hint lorem ipsum dipsum cupa"));
...
*/
	/*@Bean
	public CommandLineRunner boardDemo(BoardRepository repository) {
		return (args) -> {
			// Board board1 = new Board("neutrino");
			...
			Board board2 = new Board("no problem");
			board2.addUserToBoard(new User("arek", "password"));
